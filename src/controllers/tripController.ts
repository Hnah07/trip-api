...
export const getAllTrips = async (req: Request, res: Response) => {
  try {
    const trips = await Trip.find().populate("expenses"); //populate is a method that populates the trips with the expenses
...
export const getTripById = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error) { //"instanceof" is a method that checks if the error is an instance of Error
      res.status(500).json({ message: error.message });
    } else {
      res.status(500).json({ message: "Something went wrong" });
    }
